...
from app.screener.ranker import rank_contracts
...
def test_fundamentals_filter_fails_pe() -> None:
    assert FundamentalsFilter().passes(_make_quote(pe_ratio=60.0)) is False


def test_fundamentals_filter_fails_mktcap() -> None:
    assert FundamentalsFilter().passes(_make_quote(market_cap=1_000_000_000)) is False
