...
export default function FilterRail({ params, onChange, onRun }: Props) {
  return (
    <aside className="w-56 shrink-0 bg-gray-50 border-r border-gray-200 p-4 flex flex-col gap-5">
      <div>
        ...
        <div className="flex gap-2">
          <input type="number" value={params.min_dte ?? 21} min={1} max={params.max_dte}
            className="w-full border border-gray-300 rounded px-2 py-1 text-sm"
            onChange={e => onChange({ ...params, min_dte: Number(e.target.value) })} />
          <span className="text-gray-400 self-center">–</span>
          <input type="number" value={params.max_dte ?? 45} min={params.min_dte} max={365}
            className="w-full border border-gray-300 rounded px-2 py-1 text-sm"
      ...
      <div>
        ...
        <input type="number" value={params.max_pe ?? 50} min={1} max={500}
          className="w-full border border-gray-300 rounded px-2 py-1 text-sm"
      ...
      <div>
        ...
        <input type="number" value={((params.min_market_cap ?? 5e9) / 1e9).toFixed(0)} min={0}
          className="w-full border border-gray-300 rounded px-2 py-1 text-sm"
      ...
      <div>
        ...
        <input type="text" placeholder="default universe" value={params.tickers ?? ''}
          className="w-full border border-gray-300 rounded px-2 py-1 text-sm"
